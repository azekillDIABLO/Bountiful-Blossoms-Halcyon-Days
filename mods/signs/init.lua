...
minetest.register_node(":default:sign_wall", {
	...
	tiles = {"default_wood.png^[brighten"},
	groups = sign_groups,
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if wdir == 0 then
			minetest.env:add_node(above, {name = "signs:sign_hanging", param2 = fdir})
...
generate_line = function(s, ypos)
	...
	local texture = ""
	local xpos = math.floor((SIGN_WITH - 2 * SIGN_PADDING - width) / 2 + SIGN_PADDING) - 8
	for i = 1, #parsed do
