...
local get_ambience = function(player)
	...
	if nod_head == "default:water" or nod_head == "default:water_flowing" then
		return {underwater=underwater}
	end

	if nod_feet == "default:water" or nod_feet == "default:water_flowing" then
	...
	tempy = minetest.find_nodes_in_area({x=pos.x-6,y=pos.y-2, z=pos.z-6}, {x=pos.x+6,y=pos.y+2, z=pos.z+6},
		{"default:lava_flowing", "default:lava", "default:water_flowing", "default:water", 
		"default:sand", "default:stone",})

	-- count separate instances in block
	for _, npos in ipairs(tempy) do
		local node = minetest.get_node(npos).name
		if node == "default:lava_flowing" or node == "default:lava" then num_lava = num_lava + 1 end
		if node == "default:water_flowing" then num_water_flowing = num_water_flowing + 1 end
		if node == "default:water" then num_water_source = num_water_source + 1 end
		if node == "default:sand" or node == "default:stone" then num_desert = num_desert + 1 end
